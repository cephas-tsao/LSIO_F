...
Partial Class management_BDP080A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        Dim usr_idno As New BoundField()
        Dim usr_pass As New BoundField()
        Dim grp_code As New BoundField()
        Dim usr_tel1 As New BoundField()
        Dim usr_tel2 As New BoundField()
        Dim usr_tel3 As New BoundField()
        Dim usr_mail As New BoundField()
        Dim Limit_type As New BoundField()
        Dim is_use As New BoundField()
        Dim usr_memo As New BoundField()

        '指定資料來源欄位
        usr_code.DataField = "usr_code"
        usr_name.DataField = "usr_name"
        usr_idno.DataField = "usr_idno"
        usr_pass.DataField = "usr_pass"
        grp_code.DataField = "grp_code"
        usr_tel1.DataField = "usr_tel1"
        usr_tel2.DataField = "usr_tel2"
        usr_tel3.DataField = "usr_tel3"
        usr_mail.DataField = "usr_mail"
        Limit_type.DataField = "Limit_type"
        is_use.DataField = "is_use"
        usr_memo.DataField = "usr_memo"

        '設定欄位標頭名稱
        usr_code.HeaderText = "使用者ID"
        usr_name.HeaderText = "使用者名稱"
        'usr_idno.HeaderText = "身份證字號"
        'usr_pass.HeaderText = "密碼"
        grp_code.HeaderText = "角色ID"
        'usr_tel1.HeaderText = "電話一"
        'usr_tel2.HeaderText = "電話二"
        'usr_tel3.HeaderText = "電話三"
        usr_mail.HeaderText = "Mail"
        'Limit_type.HeaderText = "權限類別"
        is_use.HeaderText = "是否使用"
        'usr_memo.HeaderText = "備註"

        '設定欄位是否自動換行(不換行的再設定)
        'usr_memo.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        usr_code.SortExpression = "usr_code"
        usr_name.SortExpression = "usr_name"
        'usr_idno.SortExpression = "usr_idno"
        'usr_pass.SortExpression = "usr_pass"
        grp_code.SortExpression = "grp_code"
        'usr_tel1.SortExpression = "usr_tel1"
        'usr_tel2.SortExpression = "usr_tel2"
        'usr_tel3.SortExpression = "usr_tel3"
        usr_mail.SortExpression = "usr_mail"
        'Limit_type.SortExpression = "Limit_type"
        is_use.SortExpression = "is_use"
        'usr_memo.SortExpression = "usr_memo"

        '欄位寬度(%):要留6%給編輯圖示
        usr_code.ItemStyle.Width = Unit.Percentage(15)
        usr_name.ItemStyle.Width = Unit.Percentage(20)
        grp_code.ItemStyle.Width = Unit.Percentage(10)
        usr_mail.ItemStyle.Width = Unit.Percentage(40)
        is_use.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標頭顏色
        usr_code.HeaderStyle.CssClass = "grid1"
        usr_name.HeaderStyle.CssClass = "grid1"
        usr_idno.HeaderStyle.CssClass = "grid1"
        usr_pass.HeaderStyle.CssClass = "grid1"
        grp_code.HeaderStyle.CssClass = "grid1"
        usr_tel1.HeaderStyle.CssClass = "grid1"
        usr_tel2.HeaderStyle.CssClass = "grid1"
        usr_tel3.HeaderStyle.CssClass = "grid1"
        usr_mail.HeaderStyle.CssClass = "grid1"
        Limit_type.HeaderStyle.CssClass = "grid1"
        is_use.HeaderStyle.CssClass = "grid1"
        usr_memo.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊

        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_idno.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_pass.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        grp_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_tel1.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_tel2.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_tel3.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_mail.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Limit_type.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        is_use.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_memo.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        ...
        GridView1.Columns.Add(grp_code)
        GridView1.Columns.Add(usr_mail)
        GridView1.Columns.Add(is_use)
        'GridView1.Columns.Add(usr_idno)
        'GridView1.Columns.Add(usr_pass)
        'GridView1.Columns.Add(usr_tel1)
        'GridView1.Columns.Add(usr_tel2)
        'GridView1.Columns.Add(usr_tel3)
        'GridView1.Columns.Add(Limit_type)
        'GridView1.Columns.Add(usr_memo)
    End Sub
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"usr_code"}
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.DataRow
                If Chk_Email(e.Row.Cells(6).Text) = False Then e.Row.Cells(6).Text &= ""
