...
Partial Class basic_LSI013A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        att_area.SortExpression = "att_areas"
        att_add1.SortExpression = "att_add1"
        pet_date.SortExpression = "pet_date"

        '欄位寬度(%):要留6%給編輯圖示
        fix_code.ItemStyle.Width = Unit.Percentage(15)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        com_name.ItemStyle.Width = Unit.Percentage(20)
        att_area.ItemStyle.Width = Unit.Percentage(20)
        att_add1.ItemStyle.Width = Unit.Percentage(20)
        pet_date.ItemStyle.Width = Unit.Percentage(15)
        ...
        att_add1.ItemStyle.HorizontalAlign = HorizontalAlign.Center
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            Set_CKL_Option("fix_type", "", ckl_fix_type, "", "", "B")
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If TxtFind.Text <> "" Then
            txtSubWhere.Text = " WHERE " & ddlField.SelectedValue & " LIKE N'%" & RepSql(TxtFind.Text) & "%'  and"
        Else
            txtSubWhere.Text = " WHERE 1=1  and"
        End If

        If Get_CKL_Click(ckl_fix_type) <> "" Or chkEng.checked = True Or chktoher.checked = True Then

            Dim sfix = Get_CKL_Click(ckl_fix_type).split("|")
            Dim txtSubWhere2 As String = " ("
            For i = 0 To ubound(sfix)
                txtSubWhere2 = txtSubWhere2 & " FIX_WORK.fix_type LIKE N'%" & sfix(i) & "%' or"
            Next
            txtSubWhere.Text = txtSubWhere.Text & txtSubWhere2

            If chkEng.checked = True Then
                txtSubWhere.Text = txtSubWhere.Text & " att_chk = 'Y'   or"
            End If

            If chktoher.checked = True Then
                txtSubWhere.Text = txtSubWhere.Text & " att_chkoth = 'Y'   or"
            End If
            txtSubWhere.Text = left(txtSubWhere.Text, len(txtSubWhere.Text) - 2) & ")   or"
        ...
        txtSubWhere.Text = left(txtSubWhere.Text, len(txtSubWhere.Text) - 5)
        'response.write(txtSubWhere.Text)
