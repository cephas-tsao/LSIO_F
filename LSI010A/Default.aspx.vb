...
Partial Class basic_LSI010A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim start_code As New BoundField()
        Dim usr_code As New BoundField()
        Dim eng_name As New BoundField()
        Dim stop_number As New BoundField()
        Dim att_name As New BoundField()
        Dim att_tel1 As New BoundField()

        '指定資料來源欄位
        start_code.DataField = "start_code"
        usr_code.DataField = "usr_code"
        eng_name.DataField = "eng_name"
        stop_number.DataField = "stop_number"
        att_name.DataField = "att_name"
        att_tel1.DataField = "att_tel1"

        '設定欄位標頭名稱
        start_code.HeaderText = "資料編號"
        usr_code.HeaderText = "建立者編號"
        eng_name.HeaderText = "工程名稱"
        stop_number.HeaderText = "停工文號"
        att_name.HeaderText = "聯絡人"
        att_tel1.HeaderText = "聯絡人電話"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        start_code.SortExpression = "start_code"
        usr_code.SortExpression = "usr_code"
        eng_name.SortExpression = "eng_name"
        stop_number.SortExpression = "stop_number"
        att_name.SortExpression = "att_name"
        att_tel1.SortExpression = "att_tel1"

        '欄位寬度(%):要留6%給編輯圖示
        start_code.ItemStyle.Width = Unit.Percentage(12)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        eng_name.ItemStyle.Width = Unit.Percentage(23)
        stop_number.ItemStyle.Width = Unit.Percentage(20)
        att_name.ItemStyle.Width = Unit.Percentage(20)
        att_tel1.ItemStyle.Width = Unit.Percentage(15)

        '設定欄位標題顏色
        start_code.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        eng_name.HeaderStyle.CssClass = "grid1"
        stop_number.HeaderStyle.CssClass = "grid1"
        att_name.HeaderStyle.CssClass = "grid1"
        att_tel1.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        start_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        eng_name.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        stop_number.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        att_name.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        att_tel1.ItemStyle.HorizontalAlign = HorizontalAlign.Left

        '設定欄位是否顯示(不顯示的再設定)
        'Com_name.Visible = False
        GridView1.Columns.Add(start_code)
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(eng_name)
        GridView1.Columns.Add(stop_number)
        GridView1.Columns.Add(att_name)
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridView1.RowDeleting
        ...
        If Not txtDataKey = "" Then
            ...
            '刪除檔案
            Dim Dt_tmp As DataTable = Get_DataSet(s_prgcode, "Detail", txtDataKey).Tables(0)
            Dim path1 As String = Get_SysPara("FileUpload") & Trim(Dt_tmp.Rows(0).Item("start_path1") & "")
            Dim path2 As String = Get_SysPara("FileUpload") & Trim(Dt_tmp.Rows(0).Item("start_path2") & "")
    ...
    End Sub
		...
		Protected Sub bt_odf_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bt_odf.Click
        bPrint = True '開始輸出Excel
