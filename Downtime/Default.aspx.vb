...
Partial Class Downtime_Default
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim s_prgcode As String = "Downtime"
    ...
    Protected Sub setFields()
        ...
        Dim Down_code As New BoundField()
        Dim Down_institutions As New BoundField()
        Dim Down_title As New BoundField()
        Dim Down_address As New BoundField()
        'Dim Down_X As New BoundField()
        'Dim Down_Y As New BoundField()
        Dim Down_stopDate As New BoundField()
        Dim Down_returnDate As New BoundField()
        'Dim Down_range As New BoundField()
        'Dim Down_reason As New BoundField()
        'Dim Down_remarks As New BoundField()
        'Dim Down_sDate As New BoundField()
        'Dim Down_eDate As New BoundField()
        'Dim Down_pDate As New BoundField()
        'Dim Down_cDate As New BoundField()
        'Dim Down_uDate As New BoundField()
        'Dim Down_creatorID As New BoundField()
        'Dim Down_modifyID As New BoundField()
        'Dim Down_creatorIP As New BoundField()
        'Dim Down_modifyIP As New BoundField()

        '指定資料來源欄位
        usr_code.DataField = "usr_code"
        Down_code.DataField = "Down_code"
        Down_institutions.DataField = "Down_institutions"
        Down_title.DataField = "Down_title"
        Down_address.DataField = "Down_address"
        'Down_X.DataField = "Down_X"
        'Down_Y.DataField = "Down_Y"
        Down_stopDate.DataField = "Down_stopDate"
        Down_returnDate.DataField = "Down_returnDate"
        'Down_range.DataField = "Down_range"
        'Down_reason.DataField = "Down_reason"
        'Down_remarks.DataField = "Down_remarks"
        'Down_sDate.DataField = "Down_sDate"
        'Down_eDate.DataField = "Down_eDate"
        'Down_pDate.DataField = "Down_pDate"
        'Down_cDate.DataField = "Down_cDate"
        'Down_uDate.DataField = "Down_uDate"
        'Down_creatorID.DataField = "Down_creatorID"
        'Down_modifyID.DataField = "Down_modifyID"
        'Down_creatorIP.DataField = "Down_creatorIP"
        'Down_modifyIP.DataField = "Down_modifyIP"

        '設定欄位標頭名稱
        usr_code.HeaderText = "建立者編號"
        Down_code.HeaderText = "資料編號"
        Down_institutions.HeaderText = "事業單位名稱"
        Down_title.HeaderText = "工程名稱"
        Down_address.HeaderText = "停工地點"
        'Down_X.HeaderText = "X座標"
        'Down_Y.HeaderText = "Y座標"
        Down_stopDate.HeaderText = "停工日期"
        Down_returnDate.HeaderText = "復工日期"
        'Down_range.HeaderText = "停工範圍"
        'Down_reason.HeaderText = "停工原因"
        'Down_remarks.HeaderText = "備註"
        'Down_sDate.HeaderText = "置頂開始日期"
        'Down_eDate.HeaderText = "置頂結束日期"
        'Down_pDate.HeaderText = "發布日期"
        'Down_cDate.HeaderText = "建立日期"
        'Down_uDate.HeaderText = "修改日期"
        'Down_creatorID.HeaderText = "建立者ID"
        'Down_modifyID.HeaderText = "修改者ID"
        'Down_creatorIP.HeaderText = "建立者IP"
        'Down_modifyIP.HeaderText = "修改者IP"
        ...
        Down_code.ItemStyle.Width = Unit.Percentage(5)
        Down_institutions.ItemStyle.Width = Unit.Percentage(15)
        Down_title.ItemStyle.Width = Unit.Percentage(26)
        Down_address.ItemStyle.Width = Unit.Percentage(26)
        Down_stopDate.ItemStyle.Width = Unit.Percentage(11)
        Down_returnDate.ItemStyle.Width = Unit.Percentage(11)

        '設定欄位標題顏色
        Down_code.HeaderStyle.CssClass = "grid1"
        Down_institutions.HeaderStyle.CssClass = "grid1"
        Down_title.HeaderStyle.CssClass = "grid1"
        Down_address.HeaderStyle.CssClass = "grid1"
        Down_stopDate.HeaderStyle.CssClass = "grid1"
        Down_returnDate.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        'usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Down_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Down_stopDate.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Down_returnDate.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        'pet_date.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'Down_X.Visible = False
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(Down_code)
        GridView1.Columns.Add(Down_institutions)
        GridView1.Columns.Add(Down_title)
        GridView1.Columns.Add(Down_address)
        GridView1.Columns.Add(Down_stopDate)
        GridView1.Columns.Add(Down_returnDate)
        usr_code.Visible = False
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.DataRow
                '停復工日期 轉 國曆
                'Dim y As Integer = Val(Left(e.Row.Cells(10).Text, 4)) - 1911
                'If 1 = 1 Then e.Row.Cells(10).Text = y & Right(e.Row.Cells(10).Text, 6)
                'y = Val(Left(e.Row.Cells(11).Text, 4)) - 1911
                'If 1 = 1 Then e.Row.Cells(11).Text = y & Right(e.Row.Cells(11).Text, 6)
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If TxtFind.Text <> "" Then
            txtSubWhere.Text = " WHERE " & ddlField.SelectedValue & " LIKE N'%" & RepSql(Sanitizer.GetSafeHtmlFragment(AntiXss.HtmlEncode(TxtFind.Text))) & "%'"
        Else
