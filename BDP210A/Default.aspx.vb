...
Partial Class management_BDP210A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim Code_code As New BoundField()
        Dim Code_name As New BoundField()
        Dim Scr_no As New BoundField()
        Dim Field_code As New BoundField()
        Dim Field_name As New BoundField()
        Dim Code_type As New BoundField()
        Dim bdp210 As New BoundField()
        Dim is_use As New BoundField()


        '指定資料來源欄位
        Code_code.DataField = "Code_code"
        Code_name.DataField = "Code_name"
        Scr_no.DataField = "Scr_no"
        Field_code.DataField = "Field_code"
        Field_name.DataField = "Field_name"
        Code_type.DataField = "Code_type"
        bdp210.DataField = "bdp210"
        is_use.DataField = "is_use"

        '設定欄位標頭名稱
        Code_code.HeaderText = "代碼代號"
        Code_name.HeaderText = "代碼名稱"
        Scr_no.HeaderText = "顯示順序"
        Field_code.HeaderText = "選項值"
        Field_name.HeaderText = "選項中文"
        Code_type.HeaderText = "類別"
        bdp210.HeaderText = "序號"
        is_use.HeaderText = "是否使用"
        '設定欄位是否自動換行
        'Code_code.ItemStyle.Wrap = False
        'Code_name.ItemStyle.Wrap = False
        'Scr_no.ItemStyle.Wrap = False
        'Field_code.ItemStyle.Wrap = False
        'Field_name.ItemStyle.Wrap = False
        'Code_type.ItemStyle.Wrap = False
        'bdp210.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        Code_code.SortExpression = "Code_code"
        Code_name.SortExpression = "Code_name"
        Scr_no.SortExpression = "Scr_no"
        Field_code.SortExpression = "Field_code"
        Field_name.SortExpression = "Field_name"
        Code_type.SortExpression = "Code_type"
        bdp210.SortExpression = "bdp210"
        is_use.SortExpression = "is_use"
        '欄位寬度(%):要留6%給編輯圖示
        Code_code.ItemStyle.Width = Unit.Percentage(10)
        Code_name.ItemStyle.Width = Unit.Percentage(15)
        Scr_no.ItemStyle.Width = Unit.Percentage(10)
        Field_code.ItemStyle.Width = Unit.Percentage(15)
        Field_name.ItemStyle.Width = Unit.Percentage(30)
        Code_type.ItemStyle.Width = Unit.Percentage(10)
        is_use.ItemStyle.Width = Unit.Percentage(10)
        '設定欄位標題顏色
        Code_code.HeaderStyle.CssClass = "grid1"
        Code_name.HeaderStyle.CssClass = "grid1"
        Scr_no.HeaderStyle.CssClass = "grid1"
        Field_code.HeaderStyle.CssClass = "grid1"
        Field_name.HeaderStyle.CssClass = "grid1"
        Code_type.HeaderStyle.CssClass = "grid1"
        bdp210.HeaderStyle.CssClass = "grid1"
        is_use.HeaderStyle.CssClass = "grid1"
        '設定欄位的水平對齊
        Code_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Code_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Scr_no.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Field_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Field_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Code_type.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        bdp210.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        is_use.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        '設定欄位是否顯示(不顯示的再設定)
        'dep_code.Visible = False

        GridView1.Columns.Add(Code_code)
        GridView1.Columns.Add(Code_name)
        GridView1.Columns.Add(Scr_no)
        GridView1.Columns.Add(Field_code)
        GridView1.Columns.Add(Field_name)
        GridView1.Columns.Add(Code_type)
        GridView1.Columns.Add(bdp210)
        GridView1.Columns.Add(is_use)
        bdp210.Visible = False
    End Sub
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.Header Or e.Row.RowType = DataControlRowType.DataRow Then
            ...
            End If
        End If
        Dim is_use = e.Row.Cells(10).Text
        If is_use Is Nothing = False And e.Row.RowIndex > -1 Then
            If is_use = "1" Then
