...
Partial Class basic_LSI005A_Default2
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim ord_code As New BoundField()
        Dim usr_code As New BoundField()
        Dim per_mail As New BoundField()
        Dim per_type As New BoundField()
        Dim com_name As New BoundField()
        Dim pet_date As New BoundField()

        '指定資料來源欄位
        ord_code.DataField = "ord_code"
        usr_code.DataField = "usr_code"
        per_mail.DataField = "per_mail"
        per_type.DataField = "per_types"
        com_name.DataField = "com_name"
        pet_date.DataField = "pet_date"

        '設定欄位標頭名稱
        ord_code.HeaderText = "資料編號"
        usr_code.HeaderText = "建立者編號"
        per_mail.HeaderText = "電子郵件"
        per_type.HeaderText = "身份"
        com_name.HeaderText = "事業單位/姓名"
        pet_date.HeaderText = "申請日期"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        ord_code.SortExpression = "ord_code"
        usr_code.SortExpression = "usr_code"
        per_mail.SortExpression = "per_mail"
        per_type.SortExpression = "per_types"
        com_name.SortExpression = "com_name"
        pet_date.SortExpression = "pet_date"

        '欄位寬度(%):要留6%給編輯圖示
        ord_code.ItemStyle.Width = Unit.Percentage(12)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        per_mail.ItemStyle.Width = Unit.Percentage(20)
        per_type.ItemStyle.Width = Unit.Percentage(10)
        com_name.ItemStyle.Width = Unit.Percentage(30)
        pet_date.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        ord_code.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        per_mail.HeaderStyle.CssClass = "grid1"
        per_type.HeaderStyle.CssClass = "grid1"
        com_name.HeaderStyle.CssClass = "grid1"
        pet_date.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        ord_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        per_mail.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        per_type.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        com_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        pet_date.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'per_mail.Visible = False
        'GridView1.Columns.Add(ord_code)
        'GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(com_name)
        GridView1.Columns.Add(per_mail)
        'GridView1.Columns.Add(per_type)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"ord_code"}
            GridView1.DataKeyNames = KeyNames
            ws.InsUsrRec(s_prgname, "SRC", s_prgcode, Session("usr_code"), Session("usr_ip"), "", "", "")
            txtOrder.Text = " ORDER BY ord_code DESC"

            txt_sch_code.Text = Request("sch_code")
    ...
    Protected Sub setGridViewData()
        ...
        Else
            SqlDS1.SelectCommand = Get_SqlStr(s_prgcode, "SELECT", "WHERE 1=1")
        End If
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        For i = 0 To GridView1.Rows.Count - 1
            SaveEditData("LSI005A_detail", "ADD", "", Request("txt_sch_code"), GridView1.Rows(i).Cells(4).Text, "使用者自訂")
        Next
        Me.Page.Form.Controls.Add(New LiteralControl("<script>alert('存檔完成');window.close();</script>"))
