...
Partial Class basic_LSI008A_FileUpload
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim sSavePath As String = Get_SysPara("FileUpload") & "LSI008A_tmp.xls"
    ...
    Protected Function Update_ExcelFile() As Boolean
        ...
        Dim usr_dep As String = ""
        Dim usr_mail As String = ""
        '計算結果
        ...
        Try
            For Each row As DataRow In dsTmp.Tables(0).Rows
                usr_code = row.Item("使用者編號").ToString.Trim
                usr_name = row.Item("使用者名稱").ToString.Trim
                usr_dep = row.Item("科室名稱").ToString.Trim
                usr_mail = row.Item("連絡mail").ToString.Trim
                If usr_code = "" And usr_name = "" And usr_dep = "" And usr_mail = "" Then Continue For
                '計算總筆數
                iTotalCount += 1

                If Chk_RelData("LSI008A_detail", "", MrdCode.Text, usr_code, usr_name, usr_mail, usr_dep) = True Then
                    If SaveEditData("LSI008A_detail", "ADD", "", MrdCode.Text, usr_code, usr_name, usr_mail, usr_dep) = False Then
                        iFailCount += 1
