...
Partial Class basic_LSI026_edit
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If SaveEditData(s_prgcode, Pgm_Type, txt_pc_code.Text, txt_com_name.Text, txt_att_tel1.Text, txt_att_name1.Text, txt_att_mail.Text, txt_att_cell_phone.Text, txt_pc_date.Text, _
                 txt_eng_name.Text, txt_eng_code.Text, ddl_eng_area.SelectedValue, ddl_eng_road.SelectedValue, txt_eng_add.Text, txt_eng_floor.Text, pc_date_s, pc_date_e, Get_CKL_Click(chk_pc_kind1), _
				 Get_CKL_Click(chk_pc_kind2)) Then
    ...
    Protected Function Chk_data() As Boolean
        ...
        If RepSql(txt_com_name.Text) = "" Or RepSql(txt_att_name1.Text) = "" Or RepSql(txt_eng_name.Text) = "" Or RepSql(txt_eng_add.Text) = "" _
                                  Or (Chk_chk_pc_kind1() = False And Chk_chk_pc_kind2() = False) Or txt_att_mail.Text = "" Or txt_att_cell_phone.Text = "" _
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Set_CKL_Option("pc_kind1", "", chk_pc_kind1, "", "", "B")
			Set_CKL_Option("pc_kind2", "", chk_pc_kind2, "", "", "B")
            Set_Date_DDL()
            ...
            '編輯模式
            txtPgmType.Text = System.Web.HttpUtility.HtmlEncode(CType(PreviousPage.FindControl("txtPgmType"), TextBox).Text)
            '首頁條件
            txtSubWhere.Text = System.Web.HttpUtility.HtmlEncode(CType(PreviousPage.FindControl("txtsubwhere"), TextBox).Text)
            txtOrder.Text = System.Web.HttpUtility.HtmlEncode(CType(PreviousPage.FindControl("txtOrder"), TextBox).Text)
            '非新增模式則預設取得資料
            Dim Pgm_type As String = txtPgmType.Text
            txtPrimary.Text = System.Web.HttpUtility.HtmlEncode(CType(PreviousPage.FindControl("txtPrimary"), TextBox).Text)
            ...
            Select Case Pgm_type
                ...
                Case "MDY", "COPY"
                    ...
                    '                  Set_CKL_Click(Trim(Dt_tmp.Rows(0).Item("pc_kind1") & ""), chk_pc_kind1, "pc_kind1")
                    'Set_CKL_Click(Trim(Dt_tmp.Rows(0).Item("pc_kind2") & ""), chk_pc_kind2, "pc_kind2")
                    ...
                    '              End If
                    If Dt_tmp.Rows.Count > 0 Then
                        Dim r As DataRow = Dt_tmp.Rows(0)

                        txt_pc_code.Text = SafeText(r("pc_code"))
                        txt_eng_name.Text = SafeText(r("eng_name"))

                        ' 區域/道路下拉
                        SetSelectedValueSafely(ddl_eng_area, r("eng_area"))
                        ddl_eng_road.Items.Clear()
                        Set_DDL_Option(ddl_eng_area.SelectedValue, "", ddl_eng_road, "", "---請選擇---", "B")
                        SetSelectedValueSafely(ddl_eng_road, r("eng_road"))

                        txt_eng_add.Text = SafeText(r("eng_add"))
                        txt_att_name1.Text = SafeText(r("att_name1"))
                        txt_att_tel1.Text = SafeText(r("att_tel1"))
                        txt_com_name.Text = SafeText(r("com_name"))
                        txt_att_cell_phone.Text = SafeText(r("att_cell_phone"))
                        txt_att_mail.Text = SafeText(r("att_mail"))
                        txt_pc_date.Text = SafeText(r("pc_date"))

                        ' CheckBoxList
                        Set_CKL_Click(SafeText(r("pc_kind1")), chk_pc_kind1, "pc_kind1")
                        Set_CKL_Click(SafeText(r("pc_kind2")), chk_pc_kind2, "pc_kind2")

                        ' 開始日期
                        Dim sSpilt As String() = SafeDateSplit(r("pc_date_s"))
                        If sSpilt(0) <> "" Then ddl_year_s.SelectedValue = sSpilt(0)
                        If sSpilt(1) <> "" Then ddl_month_s.SelectedValue = sSpilt(1)
                        If sSpilt(2) <> "" Then ddl_date_s.SelectedValue = sSpilt(2)

                        ' 結束日期
                        Dim eSpilt As String() = SafeDateSplit(r("pc_date_e"))
                        If eSpilt(0) <> "" Then ddl_year_e.SelectedValue = eSpilt(0)
                        If eSpilt(1) <> "" Then ddl_month_e.SelectedValue = eSpilt(1)
                        If eSpilt(2) <> "" Then ddl_date_e.SelectedValue = eSpilt(2)

                        txt_eng_floor.Text = SafeText(r("eng_floor"))
                        txt_eng_code.Text = SafeText(r("eng_code"))
                    End If
        ...
        l_ErrMsg.Text = ""
    End Sub
    ' 文字輸出用：HTML Encode，避免 XSS
    Private Function SafeText(ByVal v As Object) As String
        Dim s As String = Trim((If(v, "")).ToString())
        Return System.Web.HttpUtility.HtmlEncode(s)
    End Function

    ' DropDownList 安全選取：只有當值存在於 Items 時才賦值
    Private Sub SetSelectedValueSafely(ByVal ddl As DropDownList, ByVal v As Object)
        Dim s As String = Trim((If(v, "")).ToString())
        Dim item As ListItem = ddl.Items.FindByValue(s)
    ...
    ' 日期格式化（可選）：轉為 yyyy/MM/dd；失敗則回空白
    Private Function SafeDate(ByVal v As Object) As String
        Dim s As String = Trim((If(v, "")).ToString())
        Dim d As DateTime
    ...
    ' 日期安全切割，回傳 {年,月,日}
    Private Function SafeDateSplit(ByVal v As Object) As String()
        Dim s As String = Trim((If(v, "")).ToString())
    ...
    Protected Sub Bt_send_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Bt_send.Click
        ...
        Dim sBody As String = "" & _
                ...
                "</td>" & _
                "<tr>" & _
                "  <td colspan='4'>種類：</td>" & _
                "  <td colspan='8' VALIGN='top'>丁類危險性工作場所主要危害作業：<br>" & pckind1list & "</td>" & _
				"  <td colspan='8' VALIGN='top'>機械作業：<br>" & pckind2list & "</td>" & _
                "</tr>" & _
                "</table>"
