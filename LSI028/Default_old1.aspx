 ...
 Dim pic2,seq1,seq2,seq3,seq4,seq5,seq6,seq7,seq8,seqtt
  ...
  Dim IDA As String = Request("ID") 
   ...
   Dim SqlStra1 As String
       ...
       SqlStra1  = "SELECT * FROM LSI026 where pc_type =N'一般科' order by pc_code desc"
...
<input type="button" style="width: 100px; font-size: 14px; background-color:olivedrab; color: #fff;"  onclick="window.location.href='Default-1.aspx'" value="勞條科" /></td>
            <td width="55%" align="center"><input type="button"  onclick="window.location.href='Default-3.aspx'" value="各季統計" />
&nbsp;&nbsp;
...
<input type="button"  onclick="xls()" value="匯出Excel" /></td>
  ...
  <td align="center" bgcolor="#39A96B">檔案</td>
	...
	%> 
    <tr>
      <td height="44" align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%> >  <img src="icon-survey (1).png" width="37" height="37"  onclick="window.open('https://service2.lio.gov.taipei/LSIO_F/Supload/Index?code=<%=DT.Rows(NowPageCount).Item("pc_code")%>', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');"> </td>
 <td height="44" align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%> > 
<% if (DT.Rows(NowPageCount).Item("pc_file_path")<>"")  then%>
 <img src="icon-survey (1).png" width="37" height="37"  onclick="window.open('https://service2.lio.gov.taipei/LSIO_F/upload/File_LSI026/<%=DT.Rows(NowPageCount).Item("pc_file_path")%>', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');"> 
<% end if%>
</td>
