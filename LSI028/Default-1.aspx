...
function Chk_All_1() {
    ...
    <script language="javascript">
        ...
        function Chk_All_1() {
            ...
            var chk1 = document.getElementById('CBL_Field1_0');

            var ele = document.getElementsByName('chk');
            if (document.getElementById('CBL_Field1_0').checked) {
        ...
        function count_chk() {
            ...
            //alert(j);
            document.getElementById('total').innerText = "共" + j + "件";
        }
 ...
 Dim pic2,seq1,seq2,seq3,seq4,seq5,seq6,seq7,seq8,seqtt
  ...
  Dim IDA As String = Request("ID") 
   ...
   Dim SqlStra1 As String
       ...
       SqlStra1  = "SELECT * FROM LSI026 where pc_type='勞條科' order by pc_date desc"
...
<body>
	...
	<div align="center" style="width: 990px;border:1px #000000 solid;margin:0px auto;">
      ...
      <table width="100%" border="0">
        <tbody>
          <tr>
            <td width="45%" align="center"><input type="button" style="width: 100px; font-size: 14px; background-color:darkgreen; color: #fff;"  onclick="window.location.href='Default.aspx'" value="一般科" />
&nbsp;&nbsp;
<input type="button" style="width: 100px; font-size: 14px; background-color:olivedrab; color: #fff;"  onclick="window.location.href='Default-1.aspx'" value="勞條科" /></td>
               <td width="10%" align="center"><label  id="total"  for ="total" > 共<%=RecordCount%>件</label></td>
               
            <td width="45%" align="center"> <input type="button"  onclick="count_chk();" value="統計總數" /> &nbsp;&nbsp;  <input type="button"  onclick="    xls()" value="匯出Excel" /></td>
          </tr>
        </tbody>
      </table>
      <br />
      <table width="95%"   class="table2excel" border="0" align="center" cellpadding="1" cellspacing="1" >
        <tbody>
    <tr style="color: #fff;">
        <td height="30" align="center" bgcolor="#39A96B"><input id="CBL_Field1_0" type="checkbox" name="CBL_Field1_0" onclick="Chk_All_1();"/>全選</td>
      <td width="5%" height="30" align="center" bgcolor="#39A96B">通報</td>
      ...
      <td align="center" bgcolor="#39A96B">申報單位電話</td>
      <td align="center" bgcolor="#39A96B">聯繫人</td>
	  <td align="center" bgcolor="#39A96B">電子郵件</td>
	...
	%> 
             <%
                ...
                If season = "4" Then season = "四"
                 %> 
    <tr>
        <td align="center" bgcolor="#9AEB65"><input id="chk" type="checkbox" name="chk"/></td>
	  <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>>
<a href="Default-1-2.aspx?pc_code=<%=DT.Rows(NowPageCount).Item("pc_code")%>">回復事業單位</a></td>
      <td height="44" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>> 
 <img src="document-download-outline.png" width="37" height="37"  onclick="window.open('https://service2.lio.gov.taipei/LSIO_F/upload/File_LSI026/<%=DT.Rows(NowPageCount).Item("pc_file_path")%>', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');"></td>
      ...
      <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>><%=season%></td>
      <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>><%=DT.Rows(NowPageCount).Item("com_name")%></td>
      <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>><%=DT.Rows(NowPageCount).Item("att_tel1")%></td>
      <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>><%=DT.Rows(NowPageCount).Item("att_name1")%></td>
      <td align="center" <%if rowNo mod 2 = 0 then%>bgcolor="#9AEB65" <%else%> bgcolor="#D9F86B"<%end if%>><%=DT.Rows(NowPageCount).Item("att_mail")%></td>
