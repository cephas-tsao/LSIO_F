...
Partial Class basic_LSI004A_detail_edit
    ...
    Dim s_prgcode As String = "LSI004A_detail"
    ...
    Protected Sub setFields()
        ...
        Dim news_code As New BoundField()
        Dim scr_no As New BoundField()
        Dim news_type As New BoundField()

        '指定資料來源欄位
        serial_no.DataField = "serial_no"
        news_code.DataField = "news_title"
        scr_no.DataField = "scr_no"
        news_type.DataField = "news_types"


        '設定欄位標頭名稱
        news_code.HeaderText = "新聞"
        scr_no.HeaderText = "顯示順序"
        news_type.HeaderText = "新聞類型"

        '設定欄位是否自動換行
        'news_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        news_code.SortExpression = "news_title"
        scr_no.SortExpression = "scr_no"
        news_type.SortExpression = "news_types"


        '欄位寬度(%):要留6%給編輯圖示
        news_code.ItemStyle.Width = Unit.Percentage(70)
        scr_no.ItemStyle.Width = Unit.Percentage(10)
        news_type.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位顏色
        news_code.HeaderStyle.CssClass = "grid1"
        scr_no.HeaderStyle.CssClass = "grid1"
        news_type.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        news_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        scr_no.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        news_type.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        serial_no.Visible = False

        'GridView1.Columns.Add(serial_no)
        GridView1.Columns.Add(news_code)
        GridView1.Columns.Add(news_type)
    ...
    Protected Sub bt_Save_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bt_Save.Click
        ...
        If p1.SaveEditData(s_prgcode, txtPgmType.Text, txtPrimary.Text, txt_paper_code.Text, txt_news_code.Text, txt_news_type.Text, _
                           ddl_scr_no.SelectedValue) Then
            ws.InsUsrRec(txtPrimary.Text, txtPgmType.Text, s_prgcode, Session("usr_code"), Session("usr_ip"), sFieldName, sOldData, sNewData)

            '編輯新聞本文
            If txt_news_contect.Visible = True Then
                p1.SaveEditData(s_prgcode, "NewsContect", txt_news_code.Text, txt_news_contect.Text)
            End If
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            p1.Set_DDL_Option("scr_no", "", ddl_scr_no, "", "", "B")
            IB_per_code.Attributes("onclick") = ws.Set_AddForm("../../public/addForm.aspx", "ENW-1", "1|2|3", _
                                                               "detail_edit$txt_news_code|detail_edit$txt_news_title|detail_edit$txt_news_type", "")
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"serial_no"}
            GridView1.DataKeyNames = KeyNames

            '設定隱藏欄位
            txt_news_type.Attributes.Add("style", "display:none")

            '設定唯讀欄位
            txt_serial_no.Attributes.Add("ReadOnly", "ReadOnly")
            txt_news_code.Attributes.Add("ReadOnly", "ReadOnly")
            txt_news_title.Attributes.Add("ReadOnly", "ReadOnly")
        End If
        ...
        '設定編輯固定欄位
        txt_paper_code.Text = txtKeyName.Text
    ...
    Protected Sub setEditData()
        '2012/05/31 新增
        l_news_contect.Visible = False
        txt_news_contect.Visible = False
        Select Case txtPgmType.Text
            Case "MDY", "COPY"
                ...
                If Dt_tmp.Rows.Count > 0 Then
                    ...
                    ddl_scr_no.SelectedValue = Dt_tmp.Rows(0).Item("scr_no").ToString.Trim
                    txt_news_code.Text = Dt_tmp.Rows(0).Item("news_code").ToString.Trim
                    txt_news_type.Text = Dt_tmp.Rows(0).Item("news_type").ToString.Trim
                    txt_news_title.Text = ws.QueryData(txt_news_code.Text, "e_news", "news_code", "news_title")
                End If
            Case "ADD"
                '新增模式則清空資料
                initEdit()
    ...
    Protected Function Chk_data() As Boolean
        ...
        ''檢查重覆資料
        If txtPgmType.Text = "ADD" Then
            If p1.Chk_RelData(s_prgcode, "", txt_paper_code.Text, txt_news_code.Text) = False Then
                Chk_data = False
        ...
        '檢查必填欄位是否空白
        If txt_news_code.Text = "" Or ddl_scr_no.SelectedValue = "" Then
            Chk_data = False
    ...
    Public Sub Set_Paper_Html()
        ...
        Dim dtPaper As DataTable = p1.Get_DataSet(s_prgcode, "Paper", txt_paper_code.Text).Tables(0)
        '若無資料則離開
        ...
        Dim PapType As String = dtPaper.Rows(0).Item("paper_type") & ""
        Dim CSSType As String = dtPaper.Rows(0).Item("paper_css") & ""
        Dim sHtml As String = ""

        '設定基本3種CSS
        Select Case CSSType
            Case "A"
                sHtml = "<style type='text/css'>" & _
            ...
            Case "B"
                sHtml = "<style type='text/css'>" & _
                        "body{margin-left: 0px;margin-top:15px;margin-right:0px;margin-bottom:0px;}" & _
                        ".title1{font-family:微軟正黑體;font-size:x-large;color:navy;font-weight:600;line-height:normal;text-decoration:underline}" & _
                        ".title2{font-family:微軟正黑體;font-size:x-large;color:green;font-weight:600;line-height:normal;text-decoration:underline}" & _
                        ".context{font-family:微軟正黑體;font-size:x-large;color:blue;font-weight:600;line-height:normal;text-decoration:underline}" & _
                         "</style> "
            Case "C"
                sHtml = "<style type='text/css'>" & _
                        "body{margin-left: 0px;margin-top:15px;margin-right:0px;margin-bottom:0px;}" & _
                        ".title1{font-family:微軟正黑體;font-size:x-large;color:navy;font-weight:600;line-height:normal;text-decoration:underline}" & _
                        ".title2{font-family:微軟正黑體;font-size:medium;color:peru;font-weight:600;line-height:normal;text-decoration:underline}" & _
                        ".context{font-family:微軟正黑體;font-size:x-large;color:blue;font-weight:600;line-height:normal;text-decoration:underline}" & _
                         "</style> "
        End Select
        ...
        '        '即時電子報的CSS設定
        '        sHtml = "<style type='text/css'> " & _
        '                ".head {text-align: center;color:red;font-family:標楷體;font-size:x-large;font-weight:bold} " & _
        '                ".title {text-align:center;color:blue;font-family:標楷體;font-size:larger;font-weight:bold} " & _
        '                ".subtitle {font-family:標楷體;font-weight:bold;font-size:larger;text-align:left} " & _
        '                ".pictext {text-align:center;color:blue;font-family:標楷體;padding:0px 20px 0px 20px} " & _
        '                ".sentence {text-align:left;font-family:標楷體;font-size: larger} " & _
        '                ".style2 {font-family: 標楷體;font-weight: bold;font-size: larger;text-align: right} " & _
        '                ".style4 {font-family:'Times New Roman';} " & _
        '                "</style> "
        '    Case "B"
        '        '定期電子報的CSS設定
        '        sHtml = "<style type='text/css'>" & _
        ...
        Select Case PapType
            Case "A"               '即時電子報的內文
                Dim dtPaperDetail01 As DataTable = p1.Get_DataSet(s_prgcode, "InTime", txt_paper_code.Text).Tables(0)
                If dtPaperDetail01.Rows.Count > 0 Then
                    sHtml &= "<tr><td><p class='title1'>" & dtPaper.Rows(0).Item("paper_name") & "</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail01.Rows.Count - 1
                        Dim sNewsTitle As String = dtPaperDetail01.Rows(i).Item("news_title") & ""
                        ...
                        If sNewsContect <> "" Then
                            sHtml &= "<tr><td><br><p class='title2'>" & sNewsTitle & "</p></td></tr>"
                        End If

                        '內文
                        sHtml &= "<tr><td><br><p>" & Replace(sNewsContect, "", "&nbsp;") & "</p></td></tr>"

                        If sNewsPic1 <> "" Then
                            '照片
                            sHtml &= "<tr align=center><td>" & _
                                    "<img alt='照片' src='" & sWebPic & sNewsPic1 & "' width=373 height=284></td></tr>"
                        End If
                        '分割線
            ...
            Case "B"              '定期電子報的內文
                '電子報目錄
                Dim dtSummaryL As DataTable = p1.Get_DataSet(s_prgcode, "SummaryL", txt_paper_code.Text).Tables(0)
                Dim dtSummaryR As DataTable = p1.Get_DataSet(s_prgcode, "SummaryR", txt_paper_code.Text).Tables(0)
                If dtSummaryL.Rows.Count > 0 Or dtSummaryR.Rows.Count > 0 Then
                    Dim iMaxRow As Integer = dtSummaryL.Rows.Count
                    If dtSummaryR.Rows.Count > iMaxRow Then iMaxRow = dtSummaryR.Rows.Count
                    sHtml &= "<tr><td><table border ='1' width='100%'><tr align=center><td colspan=2><b>本期重點摘要</b></td></tr>"

                    For i As Integer = 0 To iMaxRow - 1
                        '左側目錄
                        If dtSummaryL.Rows.Count > i Then
                            Dim sName As String = Trim(dtSummaryL.Rows(i).Item("field_name") & "")
                            Dim sCode As String = Trim(dtSummaryL.Rows(i).Item("news_type") & "")
                            sHtml &= "<tr><td width='50%'><a href='#" & sCode & "' title='" & sName & "'>" & sName & "</a></td>"
                        Else
                            sHtml &= "<tr><td width='50%'><a href='#'></a></td>"
                        End If
                        '右側目錄
                        If dtSummaryR.Rows.Count > i Then
                            Dim sName As String = Trim(dtSummaryR.Rows(i).Item("field_name") & "")
                            Dim sCode As String = Trim(dtSummaryR.Rows(i).Item("news_type") & "")
                            sHtml &= "<td width='50%'><a href='#" & sCode & "' title='" & sName & "'>" & sName & "連結</a></td></tr>"
                        Else
                            sHtml &= "<td width='50%'><a href='#'></a></td></tr>"
                        End If
                    Next
                    sHtml &= "</table></td></tr>"
                End If
                ...
                '電子報明細-最新消息
                Dim dtPaperDetail01 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail01", txt_paper_code.Text).Tables(0)
                If dtPaperDetail01.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='01'></a><p class='title1'>最新訊息</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail01.Rows.Count - 1
                        Dim sNewsCode As String = Trim(dtPaperDetail01.Rows(i).Item("news_code") & "")
                        Dim sNewsTitle As String = Trim(dtPaperDetail01.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail01.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail01.Rows(i).Item("news_pic1") & "")

                        sHtml &= "<tr><td><br>"
                        '新聞標題
                        If sNewsContect <> "" Then
                            sHtml &= "<a name='" & sNewsCode & "'></a><p class='title2'>" & sNewsTitle & "</p>"
                        End If
                    ...
                    Next
                    '回電子報目錄
                    sHtml &= "<tr align=right><td><a href='#' title='回電子報目錄'>回電子報目錄</a></td></tr>"
                End If

                '電子報明細-職災省思
                Dim dtPaperDetail06 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail06", txt_paper_code.Text).Tables(0)
                If dtPaperDetail06.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='06'></a><p class='title1'>職災省思</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail06.Rows.Count - 1
                        Dim sNewsTitle As String = Trim(dtPaperDetail06.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail06.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail06.Rows(i).Item("news_pic1") & "")
                ...
                '電子報明細-交流園地
                Dim dtPaperDetail02 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail02", txt_paper_code.Text).Tables(0)
                If dtPaperDetail02.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='02'></a><p class='title1'>交流園地</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail02.Rows.Count - 1
                        Dim sNewsTitle As String = Trim(dtPaperDetail02.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail02.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail02.Rows(i).Item("news_pic1") & "")
                ...
                '電子報明細-法規新訊
                Dim dtPaperDetail03 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail03", txt_paper_code.Text).Tables(0)
                If dtPaperDetail03.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='03'></a><p class='title1'>法規新訊</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail03.Rows.Count - 1
                        Dim sNewsTitle As String = Trim(dtPaperDetail03.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail03.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail03.Rows(i).Item("news_pic1") & "")
                ...
                '電子報明細-訊息報導
                Dim dtPaperDetail04 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail04", txt_paper_code.Text).Tables(0)
                If dtPaperDetail04.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='04'></a><p class='title1'>訊息報導</p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail04.Rows.Count - 1
                        Dim sNewsTitle As String = Trim(dtPaperDetail04.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail04.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail04.Rows(i).Item("news_pic1") & "")
                ...
                '電子報明細-充電小站
                Dim dtPaperDetail05 As DataTable = p1.Get_DataSet(s_prgcode, "PaperDetail05", txt_paper_code.Text).Tables(0)
                If dtPaperDetail05.Rows.Count > 0 Then
                    sHtml &= "<tr><td><a name='05'></a><p class='title1'>充電小站</p><p></p></td></tr>"

                    For i As Integer = 0 To dtPaperDetail05.Rows.Count - 1
                        Dim sNewsTitle As String = Trim(dtPaperDetail05.Rows(i).Item("news_title") & "")
                        Dim sNewsContect As String = Trim(dtPaperDetail05.Rows(i).Item("news_contect") & "")
                        Dim sNewsPic1 As String = Trim(dtPaperDetail05.Rows(i).Item("news_pic1") & "")
        ...
        p1.SaveEditData(s_prgcode, "PaperHtml", txt_paper_code.Text, sHtml)
    End Sub


    '程式固定區-不要修改 以上

    Protected Sub bt_news_contect_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bt_news_contect.Click
        '取得新聞內容 2012/05/31 新增
        Dim Dt_news_contect As DataTable = p1.Get_DataSet("LSI016A", "Detail", txt_news_code.Text).Tables(0)
        If Dt_news_contect.Rows.Count > 0 Then
            txt_news_contect.Text = Trim(Dt_news_contect.Rows(0).Item("news_contect") & "")
            l_news_contect.Visible = True
            txt_news_contect.Visible = True
        End If
    End Sub
End Class
