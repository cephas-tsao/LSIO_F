...
Partial Class basic_LSI004A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        Dim paper_name As New BoundField()
        Dim paper_num As New BoundField()
        Dim usr_name As New BoundField()
        Dim ins_date As New BoundField()
        Dim preview As New BoundField()

        '指定資料來源欄位
        paper_code.DataField = "paper_code"
        usr_code.DataField = "usr_code"
        paper_name.DataField = "paper_name"
        paper_num.DataField = "paper_num"
        usr_name.DataField = "usr_name"
        ins_date.DataField = "ins_date"


        '設定欄位標頭名稱
        paper_code.HeaderText = "資料編號"
        usr_code.HeaderText = "建立者編號"
        paper_name.HeaderText = "電子報名稱"
        paper_num.HeaderText = "電子報期數"
        usr_name.HeaderText = "使用者名稱"
        ins_date.HeaderText = "建立日期"
        'preview.HeaderText = "預覽"

        '設定欄位是否自動換行
        'paper_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        paper_code.SortExpression = "paper_code"
        usr_code.SortExpression = "usr_code"
        paper_name.SortExpression = "paper_name"
        paper_num.SortExpression = "paper_num"
        usr_name.SortExpression = "usr_name"
        ins_date.SortExpression = "ins_date"

        '欄位寬度(%):要留6%給編輯圖示
        paper_code.ItemStyle.Width = Unit.Percentage(12)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        paper_name.ItemStyle.Width = Unit.Percentage(30)
        paper_num.ItemStyle.Width = Unit.Percentage(10)
        usr_name.ItemStyle.Width = Unit.Percentage(15)
        ins_date.ItemStyle.Width = Unit.Percentage(10)
        preview.ItemStyle.Width = Unit.Percentage(5)

        '設定欄位顏色
        paper_code.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        paper_name.HeaderStyle.CssClass = "grid1"
        paper_num.HeaderStyle.CssClass = "grid1"
        usr_name.HeaderStyle.CssClass = "grid1"
        ins_date.HeaderStyle.CssClass = "grid1"
        preview.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        paper_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        paper_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        paper_num.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_name.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ins_date.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        preview.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ...
        GridView1.Columns.Add(paper_name)
        GridView1.Columns.Add(paper_num)
        GridView1.Columns.Add(usr_name)
        GridView1.Columns.Add(ins_date)
        GridView1.Columns.Add(preview)
    End Sub
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"paper_code"}
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridView1.RowDeleting
        ...
        If Not txtDataKey = "" Then
            '檢查引用關聯
            Dim dt_tmp As DataTable = Get_DataSet(s_prgcode, "ePaper", txtDataKey).Tables(0)
            If dt_tmp.Rows.Count > 0 Then
                Dim sMsg As String = "此電子報已被引用在下列發送排程("
                For i As Integer = 0 To dt_tmp.Rows.Count - 1
                    If i <> 0 Then sMsg &= "、"
                    sMsg &= Trim(dt_tmp.Rows(i).Item(0) & "")
                Next
                sMsg &= ")，請先刪除引用關聯資料!"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.Header Or e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If bPrint = True Then
                ...
                If GridView1.Columns(9).Visible = True Then e.Row.Cells(9).Visible = False
                '隱藏未選擇欄位
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.DataRow
                ...
                tmpIB.ImageUrl = "../../images/icon/preview.png"
                tmpIB.Width = 24
                tmpIB.Height = 24
                tmpIB.ToolTip = "預覽"
                tmpIB.Attributes.Add("onclick", "window.open('preview.aspx?code=" & e.Row.Cells(3).Text & "','','height=768,width=1024,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no','');return false;")
                e.Row.Cells(9).Controls.Add(tmpIB)
