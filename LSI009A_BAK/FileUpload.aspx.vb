...
Partial Class basic_LSI009A_FileUpload
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim sSavePath As String = Get_SysPara("FileUpload") & "LSI009A_tmp.xls"
    ...
    Protected Function Update_ExcelFile() As Boolean
        ...
        '新增修改資料
        Dim stop_code As String = ""        '資料編號
        Dim stop_number As String = ""      '停工文號
        Dim usr_code As String = Session("usr_code")
        Dim ins_date As String = Format(Now, "yyyy/MM/dd")
        Dim stop_date As String = ""
        Dim stop_time As String = ""
        Dim stop_date_e As String = ""
        Dim stop_time_e As String = ""
        Dim com_name As String = ""
        Dim com_per As String = ""
        Dim eng_name As String = ""
        Dim eng_code As String = ""
        Dim eng_gps As String = ""
        Dim chk_date As String = ""
        Dim ver_code As String = ""
        '計算結果
        ...
        Try
            For Each row As DataRow In dsTmp.Tables(0).Rows
                iTotalCount += 1
                stop_number = row.Item("停工文號").ToString
                stop_date = row.Item("停工日期").ToString
                stop_time = row.Item("停工時間").ToString
                stop_date_e = row.Item("停工日期_止").ToString
                stop_time_e = row.Item("停工時間_止").ToString
                com_name = row.Item("停工事業單位名稱").ToString
                com_per = row.Item("代表人").ToString
                eng_name = row.Item("工程名稱").ToString
                eng_code = row.Item("建號").ToString
                eng_gps = row.Item("座落位置").ToString
                chk_date = row.Item("檢查日期").ToString
                ver_code = row.Item("驗證碼").ToString

                '自動編號
                stop_code = "SP" & Format(Now, "yyMMdd") & Right("0000" & Get_AutoIntMax("WORK_STOP", "RIGHT(stop_code, 10)", " WHERE stop_code LIKE 'SP" & Format(Now, "yyMMdd") & "%'") + 1, 4)
                If Chk_RelData("LSI009A", "stop_code", stop_code) = True And Chk_RelData("LSI009A", "stop_number", stop_number) = True Then
                    If SaveEditData("LSI009A", "ADD", stop_code, stop_number, usr_code, ins_date, stop_date _
                                , stop_time, stop_date_e, stop_time_e, com_name, com_per, eng_name _
                                , eng_code, eng_gps, chk_date, ver_code) = False Then
                        iFailCount += 1
