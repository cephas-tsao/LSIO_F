...
Partial Class management_BDP070A_edit
    Inherits PageBase
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        '設定Key欄位來源
        Dim sKeyPrimary As String = txt_grp_code.Text '*修改處*
        ...
        If SaveEditData(s_prgcode, Pgm_Type, txt_grp_code.Text, txt_grp_name.Text, ddl_is_use.SelectedValue) Then
            '儲存使用紀錄
            ...
            If Pgm_Type <> "MDY" Then
                ...
                If txtSubWhere.Text <> "" Then
                    txtSubWhere.Text &= " OR grp_code='" & RepSql(sKeyPrimary) & "' "
                Else
                    txtSubWhere.Text = " WHERE grp_code='" & RepSql(sKeyPrimary) & "' "
                End If
            End If
            '存檔正確導回default頁面
    ...
    Protected Function Chk_data() As Boolean
        ...
        If txtPgmType.Text = "ADD" Then
            If Chk_RelData(s_prgcode, "", txt_grp_code.Text) = False Then
                Chk_data = False
                ...
                Case "MDY", "COPY"
                    ...
                    If Dt_tmp.Rows.Count > 0 Then
                        If Pgm_type = "COPY" Then
                            txt_grp_code.Text = ""
                            txt_grp_code.Enabled = True
                        Else
                            txt_grp_code.Text = txtPrimary.Text
                            txt_grp_code.Enabled = False
                        End If
                        txt_grp_name.Text = Trim(Dt_tmp.Rows(0).Item("grp_name") & "")
    ...
    Protected Sub SetClientCheck()
        '網頁伺服器端檢查點建置
        txt_grp_code.Attributes("onblur") = "javascript:IsEmpty(this);"
    End Sub
