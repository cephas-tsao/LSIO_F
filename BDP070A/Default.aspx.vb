...
Partial Class management_BDP070A_Default
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim s_prgcode As String = "BDP070A"
    ...
    Protected Sub setFields()
        ...
        Dim grp_name As New BoundField()
        Dim is_use As New BoundField()

        '指定資料來源欄位
        grp_code.DataField = "grp_code"
        grp_name.DataField = "grp_name"
        is_use.DataField = "is_use"

        '設定欄位標頭名稱
        grp_code.HeaderText = "角色編號"
        grp_name.HeaderText = "角色名稱"
        is_use.HeaderText = "是否使用"

        '設定欄位是否自動換行
        'grp_code.ItemStyle.Wrap = False
        'grp_name.ItemStyle.Wrap = False
        'is_use.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        grp_code.SortExpression = "grp_code"
        grp_name.SortExpression = "grp_name"
        is_use.SortExpression = "is_use"

        '欄位寬度(%):要留6%給編輯圖示
        grp_code.ItemStyle.Width = Unit.Percentage(15)
        grp_name.ItemStyle.Width = Unit.Percentage(69)
        is_use.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        grp_code.HeaderStyle.CssClass = "grid1"
        grp_name.HeaderStyle.CssClass = "grid1"
        is_use.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        grp_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        grp_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        ...
        GridView1.Columns.Add(grp_name)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"grp_code"}
