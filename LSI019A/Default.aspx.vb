...
Partial Class basic_LSI019A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim labor_inspect As New BoundField()
        Dim usr_code As New BoundField()
        Dim chk_date1 As New BoundField()
        Dim chk_date2 As New BoundField()
        Dim com_name As New BoundField()
        Dim eng_name As New BoundField()
        Dim work_staus As New BoundField()

        '指定資料來源欄位
        labor_inspect.DataField = "labor_inspect"
        usr_code.DataField = "usr_code"
        chk_date1.DataField = "chk_date1"
        chk_date2.DataField = "chk_date2"
        com_name.DataField = "com_name"
        eng_name.DataField = "eng_name"
        work_staus.DataField = "work_staus"

        '設定欄位標頭名稱
        'labor_inspect.HeaderText = ""
        usr_code.HeaderText = "建立者編號"
        chk_date1.HeaderText = "第1次檢查日期"
        chk_date2.HeaderText = "第2次檢查日期"
        com_name.HeaderText = "事業單位名稱"
        eng_name.HeaderText = "工程名稱"
        work_staus.HeaderText = "工地狀態"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        'labor_inspect.SortExpression = "labor_inspect"
        usr_code.SortExpression = "usr_code"
        chk_date1.SortExpression = "chk_date1"
        chk_date2.SortExpression = "chk_date2"
        com_name.SortExpression = "com_name"
        eng_name.SortExpression = "eng_name"
        work_staus.SortExpression = "work_staus"

        '欄位寬度(%):要留6%給編輯圖示
        'labor_inspect.ItemStyle.Width = Unit.Percentage(20)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        chk_date1.ItemStyle.Width = Unit.Percentage(12)
        chk_date2.ItemStyle.Width = Unit.Percentage(12)
        com_name.ItemStyle.Width = Unit.Percentage(25)
        eng_name.ItemStyle.Width = Unit.Percentage(25)
        work_staus.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        'labor_inspect.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        chk_date1.HeaderStyle.CssClass = "grid1"
        chk_date2.HeaderStyle.CssClass = "grid1"
        com_name.HeaderStyle.CssClass = "grid1"
        eng_name.HeaderStyle.CssClass = "grid1"
        work_staus.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        'labor_inspect.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        chk_date1.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        chk_date2.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        com_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        eng_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        work_staus.ItemStyle.HorizontalAlign = HorizontalAlign.Left

        '設定欄位是否顯示(不顯示的再設定)
        labor_inspect.Visible = False

        GridView1.Columns.Add(labor_inspect)
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(chk_date1)
        GridView1.Columns.Add(chk_date2)
        GridView1.Columns.Add(com_name)
        GridView1.Columns.Add(eng_name)
        GridView1.Columns.Add(work_staus)
    End Sub
    ...
    Protected Sub setLimit(ByVal Climit As String)
        ...
        IB_import.Enabled = CBool(InStr(Climit, "S") > 0)
    End Sub
