...
Partial Class management_BDP190A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        Dim cnt As New BoundField()
        Dim last As New BoundField()
        Dim is_hid As New BoundField()
        '指定資料來源欄位
        bull_code.DataField = "bull_code"
        usr_name.DataField = "usr_name"
        theme.DataField = "theme1"
        Ins_date.DataField = "Ins_date"
        'Ins_time.DataField = "Ins_time"
        cnt.DataField = "cnt"
        last.DataField = "last"
        is_hid.DataField = "is_hid"

        '設定欄位標頭名稱
        bull_code.HeaderText = "編號"
        usr_name.HeaderText = "發表人"
        theme.HeaderText = "留言主題"
        Ins_date.HeaderText = "發表日期"
        'Ins_time.HeaderText = "最後回應時間"
        cnt.HeaderText = "回應數"
        last.HeaderText = "最後回應時間"
        is_hid.HeaderText = "顯示"
        ...
        theme.SortExpression = "theme1"
        Ins_date.SortExpression = "Ins_date"
        'Ins_time.SortExpression = "Ins_time"
        cnt.SortExpression = "cnt"
        last.SortExpression = "last"
        is_hid.SortExpression = "is_hid"

        '欄位寬度(%):要留6%給編輯圖示
        bull_code.ItemStyle.Width = Unit.Percentage(15)
        usr_name.ItemStyle.Width = Unit.Percentage(15)
        theme.ItemStyle.Width = Unit.Percentage(43)
        Ins_date.ItemStyle.Width = Unit.Percentage(15)
        'Ins_time.ItemStyle.Width = Unit.Percentage(10)
        cnt.ItemStyle.Width = Unit.Percentage(8)
        last.ItemStyle.Width = Unit.Percentage(12)
        ...
        cnt.HeaderStyle.CssClass = "grid1"
        last.HeaderStyle.CssClass = "grid1"
        is_hid.HeaderStyle.CssClass = "grid1"
        ...
        last.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        is_hid.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'is_hid.Visible = False
        ...
        GridView1.Columns.Add(cnt)
        GridView1.Columns.Add(last)
        GridView1.Columns.Add(is_hid)
    End Sub
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.DataRow
                If e.Row.Cells(8).Text = "N" Then
                    e.Row.Cells(4).Text &= " <img src='../../images/icon/lock.gif' />"
                End If
                '加入複製Button
                'Dim tmpIB2 As New ImageButton
                'tmpIB2.ImageUrl = "../../images/icon/copy.png"
                'tmpIB2.Attributes.Add("onclick", "window.open('copy.aspx?tn_code=" & e.Row.Cells(3).Text & "','','height=140,width=400,scrollbars=no,status=no,toolbar=no,menubar=no,location=no','');return false;")
                'e.Row.Cells(7).Controls.Add(tmpIB2)
