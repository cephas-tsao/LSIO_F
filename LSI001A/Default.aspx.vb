...
Partial Class basic_LSI001A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim ask_code As New BoundField()
        Dim usr_code As New BoundField()
        Dim ask_name As New BoundField()
        Dim ask_email As New BoundField()
        Dim ask_type As New BoundField()

        '指定資料來源欄位
        ask_code.DataField = "ask_code"
        ask_name.DataField = "ask_name"
        usr_code.DataField = "usr_code"
        ask_email.DataField = "ask_email"
        ask_type.DataField = "ask_types"

        '設定欄位標頭名稱
        ask_code.HeaderText = "資料編號"
        ask_name.HeaderText = "姓名"
        usr_code.HeaderText = "建立者編號"
        ask_email.HeaderText = "電子信箱"
        ask_type.HeaderText = "事業單位行業別"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        ask_code.SortExpression = "ask_code"
        ask_name.SortExpression = "ask_name"
        usr_code.SortExpression = "usr_code"
        ask_email.SortExpression = "ask_email"
        ask_type.SortExpression = "ask_types"

        '欄位寬度(%):要留6%給編輯圖示
        ask_code.ItemStyle.Width = Unit.Percentage(15)
        ask_name.ItemStyle.Width = Unit.Percentage(17)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        ask_email.ItemStyle.Width = Unit.Percentage(25)
        ask_type.ItemStyle.Width = Unit.Percentage(25)

        '設定欄位標題顏色
        ask_code.HeaderStyle.CssClass = "grid1"
        ask_name.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        ask_email.HeaderStyle.CssClass = "grid1"
        ask_type.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        ask_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ask_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ask_email.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        ask_type.ItemStyle.HorizontalAlign = HorizontalAlign.Left

        '設定欄位是否顯示(不顯示的再設定)
        'ask_code.Visible = False

        GridView1.Columns.Add(ask_code)
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(ask_name)
        GridView1.Columns.Add(ask_email)
        GridView1.Columns.Add(ask_type)
    End Sub
