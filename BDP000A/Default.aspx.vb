...
Partial Class management_BDP000A_Default
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim s_prgcode As String = "BDP000A"
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim bdp000 As New BoundField()
        Dim par_name As New BoundField()
        Dim par_value As New BoundField()
        Dim par_memo As New BoundField()

        '指定資料來源欄位
        bdp000.DataField = "bdp000"
        par_name.DataField = "par_name"
        par_value.DataField = "par_value"
        par_memo.DataField = "par_memo"

        '設定欄位標頭名稱
        'bdp000.HeaderText = "識別碼"
        par_name.HeaderText = "參數名稱"
        par_value.HeaderText = "參數值"
        par_memo.HeaderText = "備註"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        'bdp000.SortExpression = "bdp000"
        par_name.SortExpression = "par_name"
        par_value.SortExpression = "par_value"
        par_memo.SortExpression = "par_memo"

        '欄位寬度(%):要留6%給編輯圖示
        par_name.ItemStyle.Width = Unit.Percentage(15)
        par_value.ItemStyle.Width = Unit.Percentage(20)
        par_memo.ItemStyle.Width = Unit.Percentage(58)

        '設定欄位標題顏色
        bdp000.HeaderStyle.CssClass = "grid1"
        par_name.HeaderStyle.CssClass = "grid1"
        par_value.HeaderStyle.CssClass = "grid1"
        par_memo.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        bdp000.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        par_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        par_value.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        par_memo.ItemStyle.HorizontalAlign = HorizontalAlign.Left

        '設定欄位是否顯示(不顯示的再設定)
        bdp000.Visible = False

        GridView1.Columns.Add(bdp000)
        GridView1.Columns.Add(par_name)
        GridView1.Columns.Add(par_value)
        GridView1.Columns.Add(par_memo)
    End Sub
