...
Partial Class basic_LSI002A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        Dim cmp_code As New BoundField()
        Dim cmp_name As New BoundField()
        Dim com_sex As New BoundField()
        Dim cmp_age As New BoundField()
        Dim cmp_email As New BoundField()
        Dim cmp_memo As New BoundField()
        Dim com_Contract_c As New BoundField()
        Dim com_paymon_c As New BoundField()
        Dim com_relax_c As New BoundField()
        Dim com_child_c As New BoundField()
        Dim com_Welfar_c As New BoundField()
        Dim com_schedule As New BoundField()
        '指定資料來源欄位
        cmp_code.DataField = "cmp_code"
        ins_date.DataField = "ins_date"
        cmp_name.DataField = "cmp_name"
        'com_sex.DataField = "com_sex"
        'cmp_age.DataField = "cmp_age"
        cmp_email.DataField = "cmp_email"
        'cmp_memo.DataField = "cmp_memo"
        'com_Contract_c.DataField = "com_Contract_c"
        'com_paymon_c.DataField = "com_paymon_c"
        'com_relax_c.DataField = "com_relax_c"
        'com_child_c.DataField = "com_child_c"
        'com_Welfar_c.DataField = "com_Welfar_c"
        'com_schedule.DataField = "com_schedule"

        '設定欄位標頭名稱
        cmp_code.HeaderText = "案件編號"
        ins_date.HeaderText = "申訴日期"
        cmp_name.HeaderText = "姓名"
        'com_sex.HeaderText = "性別"
        'cmp_age.HeaderText = "年齡"
        cmp_email.HeaderText = "電子信箱"
        'cmp_memo.HeaderText = "申訴具體內容"
        'com_Contract_c.HeaderText = "勞動契約"
        'com_paymon_c.HeaderText = "工資"
        'com_relax_c.HeaderText = "工作時間、休息、休假"
        'com_child_c.HeaderText = "童工、女工"
        'com_Welfar_c.HeaderText = "保障與福利"
        'com_schedule.HeaderText = "申訴進度"
        ...
        '設定排序所對應的欄位
        cmp_code.SortExpression = "cmp_code"
        ins_date.SortExpression = "ins_date"
        cmp_name.SortExpression = "cmp_name"
        'com_sex.SortExpression = "com_sex"
        'cmp_age.SortExpression = "cmp_age"
        cmp_email.SortExpression = "cmp_email"
        'cmp_memo.SortExpression = "cmp_memo"
        'com_Contract_c.SortExpression = "com_Contract_c"
        'com_paymon_c.SortExpression = "com_paymon_c"
        'com_relax_c.SortExpression = "com_relax_c"
        'com_child_c.SortExpression = "com_child_c"
        'com_Welfar_c.SortExpression = "com_Welfar_c"
        'com_schedule.SortExpression = "com_schedule"


        '欄位寬度(%):要留6%給編輯圖示
        cmp_code.ItemStyle.Width = Unit.Percentage(20)
        ins_date.ItemStyle.Width = Unit.Percentage(20)
        cmp_name.ItemStyle.Width = Unit.Percentage(20)
        'com_sex.ItemStyle.Width = Unit.Percentage(10)
        'cmp_age.ItemStyle.Width = Unit.Percentage(10)
        cmp_email.ItemStyle.Width = Unit.Percentage(20)
        'cmp_memo.ItemStyle.Width = Unit.Percentage(20)
        'com_Contract_c.ItemStyle.Width = Unit.Percentage(10)
        'com_paymon_c.ItemStyle.Width = Unit.Percentage(10)
        'com_relax_c.ItemStyle.Width = Unit.Percentage(10)
        'com_child_c.ItemStyle.Width = Unit.Percentage(10)
        'com_Welfar_c.ItemStyle.Width = Unit.Percentage(10)
        'com_schedule.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        cmp_code.HeaderStyle.CssClass = "grid1"
        ins_date.HeaderStyle.CssClass = "grid1"
        cmp_name.HeaderStyle.CssClass = "grid1"
        'com_sex.HeaderStyle.CssClass = "grid1"
        'cmp_age.HeaderStyle.CssClass = "grid1"
        cmp_email.HeaderStyle.CssClass = "grid1"
        'cmp_memo.HeaderStyle.CssClass = "grid1"
        'com_Contract_c.HeaderStyle.CssClass = "grid1"
        'com_paymon_c.HeaderStyle.CssClass = "grid1"
        'com_relax_c.HeaderStyle.CssClass = "grid1"
        'com_child_c.HeaderStyle.CssClass = "grid1"
        'com_Welfar_c.HeaderStyle.CssClass = "grid1"
        'com_schedule.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        cmp_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ins_date.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        cmp_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_sex.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'cmp_age.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        cmp_email.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'cmp_memo.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_Contract_c.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_paymon_c.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_relax_c.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_child_c.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_Welfar_c.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        'com_schedule.ItemStyle.HorizontalAlign = HorizontalAlign.Left


        '設定欄位是否顯示(不顯示的再設定)
        'cmp_code.Visible = False
        GridView1.Columns.Add(cmp_code)
        GridView1.Columns.Add(ins_date)
        GridView1.Columns.Add(cmp_name)
        'GridView1.Columns.Add(com_sex)
        'GridView1.Columns.Add(cmp_age)
        GridView1.Columns.Add(cmp_email)
        'GridView1.Columns.Add(cmp_memo)
        'GridView1.Columns.Add(com_Contract_c)
        'GridView1.Columns.Add(com_paymon_c)
        'GridView1.Columns.Add(com_relax_c)
        'GridView1.Columns.Add(com_child_c)
        'GridView1.Columns.Add(com_Welfar_c)
        'GridView1.Columns.Add(com_schedule)
    End Sub
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.DataRow
                'If Chk_Email(e.Row.Cells(7).Text) = False Then e.Row.Cells(7).Text &= "[無效信箱]"

                'If e.Row.Cells(5).Text = "f" Then
                '    e.Row.Cells(5).Text = "女"
                'Else
                '    e.Row.Cells(5).Text = "男"
                'End If
                'If e.Row.Cells(13).Text = "01" Then
                '    e.Row.Cells(13).Text = "實施檢查中"
                'End If
                'If e.Row.Cells(13).Text = "02" Then
                '    e.Row.Cells(13).Text = "發函事業單位備妥資料至本處受檢"
                'End If
                'If e.Row.Cells(13).Text = "03" Then
                '    e.Row.Cells(13).Text = "第2次發函事業單位備妥資料至本處受檢"
                'End If
                'If e.Row.Cells(13).Text = "04" Then
                '    e.Row.Cells(13).Text = "已回復結案"
                'End If
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            is_ok = Chk_UsrLimit(Session("usr_code"), e.Row.Cells(3).Text, Session("grp_code"))
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        '資料搜尋-單一條件
        Dim sother As String = Trim(TxtFind.Text)
        If ddlField.SelectedValue = "com_sex" Or ddlField.SelectedValue = "com_schedule" Then
            If Trim(TxtFind.Text) = "男" Then
                sother = "m"
            End If
            If Trim(TxtFind.Text) = "女" Then
                sother = "f"
            End If
            If Trim(TxtFind.Text) = "實施檢查中" Then
                sother = "01"
            End If
            If Trim(TxtFind.Text) = "發函事業單位備妥資料至本處受檢" Then
                sother = "02"
            End If
            If Trim(TxtFind.Text) = "第2次發函事業單位備妥資料至本處受檢" Then
                sother = "03"
            End If
            If Trim(TxtFind.Text) = "第2已回復結案" Then
                sother = "04"
            End If
        End If
        If TxtFind.Text <> "" Then
            txtSubWhere.Text = " WHERE " & ddlField.SelectedValue & " LIKE N'%" & RepSql(sother) & "%'"
        Else
    ...
    Protected Sub IB_delete_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles IB_delete.Click
        Dim dt_del As DataTable = Get_DataSet(s_prgcode, "SRC").Tables(0)
        If dt_del.Rows.Count > 0 Then
            For i As Integer = 0 To dt_del.Rows.Count - 1
                Dim txtDataKey As String = Trim(dt_del.Rows(i).Item(0) & "")
                '設定記錄資料
                Dim sSql As String = Get_SqlStr(s_prgcode, "Detail", txtDataKey)
                Dim sFieldName As String = ws.Get_FieldName(s_prgcode)
                Dim sOldData As String = ws.Get_OldData(sSql, s_prgcode)
                Dim sNewData As String = Get_NewData(s_prgcode, "DEL")
                Get_DataSet(s_prgcode, "DEL", txtDataKey)
                ws.InsUsrRec("當事人不同意繼續利用", "DEL", s_prgcode, Session("usr_code"), Session("usr_ip"), sFieldName, sOldData, sNewData)
            Next
        End If
        Me.Page.Form.Controls.Add(New LiteralControl("<script>alert('共刪除 " & dt_del.Rows.Count & " 筆資料');</script>"))
    End Sub
End Class
