...
Partial Class management_BDP150A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        ...
        Dim ip_type As New BoundField()
        Dim ips As New BoundField()
        Dim ipe As New BoundField()
        Dim ip_name As New BoundField()

        '指定資料來源欄位
        serial_no.DataField = "serial_no"
        ip_type.DataField = "ip_types"
        ips.DataField = "ips"
        ipe.DataField = "ipe"
        ip_name.DataField = "ip_name"

        '設定欄位標頭名稱
        serial_no.HeaderText = "識別碼"
        ip_type.HeaderText = "黑白名單"
        ips.HeaderText = "開始IP"
        ipe.HeaderText = "結束IP"
        ip_name.HeaderText = "備註"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        serial_no.SortExpression = "serial_no"
        ip_type.SortExpression = "ip_types"
        ips.SortExpression = "ips"
        ipe.SortExpression = "ipe"
        ip_name.SortExpression = "ip_name"

        '欄位寬度(%):要留6%給編輯圖示
        ip_type.ItemStyle.Width = Unit.Percentage(15)
        ips.ItemStyle.Width = Unit.Percentage(15)
        ipe.ItemStyle.Width = Unit.Percentage(15)
        ip_name.ItemStyle.Width = Unit.Percentage(49)

        '設定欄位標題顏色
        serial_no.HeaderStyle.CssClass = "grid1"
        ip_type.HeaderStyle.CssClass = "grid1"
        ips.HeaderStyle.CssClass = "grid1"
        ipe.HeaderStyle.CssClass = "grid1"
        ip_name.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        serial_no.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ip_type.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ips.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ipe.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        ip_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left

        '設定欄位是否顯示(不顯示的再設定)
        serial_no.Visible = False

        'GridView1.Columns.Add(serial_no)
        GridView1.Columns.Add(ip_type)
        GridView1.Columns.Add(ips)
        GridView1.Columns.Add(ipe)
        GridView1.Columns.Add(ip_name)
    End Sub
