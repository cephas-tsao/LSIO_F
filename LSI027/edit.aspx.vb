...
Partial Class basic_LSI025_edit
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If SaveEditData(s_prgcode, Pgm_Type, txt_pc_code.Text, txt_com_name.Text, txt_att_tel1.Text, txt_att_name1.Text, txt_att_mail.Text, txt_att_cell_phone.Text, txt_pc_date.Text,
                 txt_eng_name.Text, txt_eng_code.Text, ddl_eng_area.SelectedValue, ddl_eng_road.SelectedValue, txt_eng_add.Text, txt_eng_floor.Text, pc_date_s, pc_date_e, Get_CKL_Click(chk_pc_kind1),
        ) Then
            ...
            Response.Redirect("Default.aspx?subwhere=" & HttpUtility.UrlEncode(Replace(txtSubWhere.Text, "%", "$")) & "&order=" & HttpUtility.UrlEncode(txtOrder.Text))
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            '標頭 
            Label1.Text = Server.HtmlEncode(CType(PreviousPage.FindControl("Label1"), Label).Text)
            '編輯模式
            txtPgmType.Text = Server.HtmlEncode(CType(PreviousPage.FindControl("txtPgmType"), TextBox).Text)
            '首頁條件
            ...
            Select Case Pgm_type
                ...
                Case "MDY", "COPY"
                    ...
                    If Dt_tmp.Rows.Count > 0 Then
                        txt_pc_code.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("pc_code") & ""))
                        txt_eng_name.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("eng_name") & ""))
                        ddl_eng_area.SelectedValue = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("eng_area") & ""))
                        ddl_eng_road.Items.Clear()
                        Set_DDL_Option(ddl_eng_area.SelectedValue, "", ddl_eng_road, "", "---請選擇---", "B")
                        ddl_eng_road.SelectedValue = Trim(Dt_tmp.Rows(0).Item("eng_road") & "")
                        txt_eng_add.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("eng_add") & ""))
                        txt_att_name1.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("att_name1") & ""))
                        txt_att_tel1.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("att_tel1") & ""))
                        txt_com_name.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("com_name") & ""))
                        txt_att_cell_phone.Text = Trim(Dt_tmp.Rows(0).Item("att_cell_phone") & "")
                        txt_att_mail.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("att_mail") & ""))
                        txt_pc_date.Text = Server.HtmlEncode(Trim(Dt_tmp.Rows(0).Item("pc_date") & ""))
    ...
    Protected Sub Bt_send_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Bt_send.Click
        ...
        Dim sBody As String = "" &
                ...
                "<tr bgcolor='#7EBAD1' align='center'><td colspan='20'><p><strong>塔式起重機事業單位回報通報</strong></p></td></tr>" &
                ...
                "  <td colspan='6'>" & txt_att_mail.Text & "</td>" &
                "</tr>" &
                "<tr>" &
                "  <td colspan='4'>聯絡電話：</td>" &
                "  <td colspan='6'>" & RepSql(txt_att_tel1.Text) & "</td>" &
                "  <td colspan='4'>手機：</td>" &
                "  <td colspan='6'>" & txt_att_cell_phone.Text & "</td>" &
                "</tr>" &
                "<tr>" &
                ...
                "  <td colspan='6'>" & RepSql(txt_eng_name.Text) & "</td>" &
                "  <td colspan='4'>建號：</td>" &
                "  <td colspan='6'>" & txt_eng_code.Text & "</td>" &
                "</tr>" &
                "<tr>" &
                ...
                "</tr>" &
                "<tr>" &
                "  <td colspan='4'>作業區域及樓層：</td>" &
                "  <td colspan='16'>" & RepSql(txt_eng_floor.Text) & "</td>" &
                "</tr>" &
                "<tr>" &
                "  <td colspan='4'>預計施工期間：</td>" &
                "  <td colspan='16'>起: 西元" & ddl_year_s.SelectedValue & " 年 " &
                                                 ddl_month_s.SelectedValue & " 月 " &
                                                 ddl_date_s.SelectedValue & " 日<br>" &
                                  "迄: 西元" & ddl_year_e.SelectedValue & " 年 " &
                                                 ddl_month_e.SelectedValue & " 月 " &
                                                 ddl_date_e.SelectedValue & " 日" &
                "</td>" &
                "<tr>" &
                "  <td colspan='4'>種類：</td>" &
                "  <td colspan='8' VALIGN='top'>點檢表：<br>" & pckind1list & "</td>" & _
