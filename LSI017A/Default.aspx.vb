...
Partial Class basic_LSI017A_Default
    Inherits PageBase
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim app_code As New BoundField()
        Dim usr_code As New BoundField()
        Dim version As New BoundField()
        Dim app_path As New BoundField()
        Dim app_type As New BoundField()
        Dim update_date As New BoundField()

        '指定資料來源欄位
        app_code.DataField = "app_code"
        usr_code.DataField = "usr_code"
        version.DataField = "version"
        app_path.DataField = "app_path"
        app_type.DataField = "app_types"
        update_date.DataField = "update_date"

        '設定欄位標頭名稱
        app_code.HeaderText = "資料編號"
        usr_code.HeaderText = "建立者編號"
        version.HeaderText = "版本號碼"
        app_path.HeaderText = "檔案名稱"
        app_type.HeaderText = "APP種類"
        update_date.HeaderText = "更新日期"

        '設定欄位是否自動換行(不換行的再設定)
        'prg_code.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        app_code.SortExpression = "app_code"
        usr_code.SortExpression = "usr_code"
        version.SortExpression = "version"
        app_path.SortExpression = "app_path"
        app_type.SortExpression = "app_types"
        update_date.SortExpression = "update_date"

        '欄位寬度(%):要留6%給編輯圖示
        app_code.ItemStyle.Width = Unit.Percentage(15)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        version.ItemStyle.Width = Unit.Percentage(20)
        app_path.ItemStyle.Width = Unit.Percentage(20)
        app_type.ItemStyle.Width = Unit.Percentage(20)
        update_date.ItemStyle.Width = Unit.Percentage(15)

        '設定欄位標題顏色
        app_code.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        version.HeaderStyle.CssClass = "grid1"
        app_path.HeaderStyle.CssClass = "grid1"
        app_type.HeaderStyle.CssClass = "grid1"
        update_date.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        app_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        version.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        app_path.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        app_type.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        update_date.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'version.Visible = False
        GridView1.Columns.Add(app_code)
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(version)
        GridView1.Columns.Add(app_path)
        GridView1.Columns.Add(app_type)
        GridView1.Columns.Add(update_date)
    End Sub
