...
Partial Class management_BDP230A_Default
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim s_prgcode As String = "BDP230A"
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim dep_code As New BoundField()
        Dim dep_name As New BoundField()
        Dim dep_level As New BoundField()
        Dim is_use As New BoundField()

        '指定資料來源欄位
        dep_code.DataField = "dep_code"
        dep_name.DataField = "dep_name"
        dep_level.DataField = "dep_level"
        is_use.DataField = "is_use"

        '設定欄位標頭名稱
        dep_code.HeaderText = "部門編號"
        dep_name.HeaderText = "部門名稱"
        dep_level.HeaderText = "部門層級"
        is_use.HeaderText = "是否使用"

        '設定欄位是否自動換行
        'dep_code.ItemStyle.Wrap = False
        'dep_name.ItemStyle.Wrap = False
        'is_use.ItemStyle.Wrap = False

        '設定排序所對應的欄位
        dep_code.SortExpression = "dep_code"
        dep_name.SortExpression = "dep_name"
        dep_level.SortExpression = "dep_level"
        is_use.SortExpression = "is_use"

        '欄位寬度(%):要留6%給編輯圖示
        dep_code.ItemStyle.Width = Unit.Percentage(15)
        dep_name.ItemStyle.Width = Unit.Percentage(69)
        dep_level.ItemStyle.Width = Unit.Percentage(10)
        is_use.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        dep_code.HeaderStyle.CssClass = "grid1"
        dep_name.HeaderStyle.CssClass = "grid1"
        dep_level.HeaderStyle.CssClass = "grid1"
        is_use.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        dep_code.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        dep_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        dep_level.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        is_use.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'dep_code.Visible = False

        GridView1.Columns.Add(dep_code)
        GridView1.Columns.Add(dep_name)
        GridView1.Columns.Add(dep_level)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) Then
            ...
            '設定Row的鍵值組成，具有唯一性
            Dim KeyNames() As String = {"dep_code"}
