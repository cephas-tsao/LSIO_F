...
Partial Class basic_LSI007A_Default
    Inherits PageBase

    Dim ws As New WebReference.LSIO_WebService
    Dim s_prgcode As String = "LSI007A"
    ...
    Protected Sub setFields()
        '建立資料繫結欄位
        Dim room_code As New BoundField()
        Dim usr_code As New BoundField()
        Dim Room_name As New BoundField()
        Dim Room_memo As New BoundField()
        Dim Is_use As New BoundField()

        '指定資料來源欄位
        room_code.DataField = "room_code"
        usr_code.DataField = "usr_code"
        Room_name.DataField = "Room_name"
        Room_memo.DataField = "Room_memo"
        Is_use.DataField = "Is_uses"

        '設定欄位標頭名稱
        room_code.HeaderText = "會議室編號"
        usr_code.HeaderText = "建立者編號"
        Room_name.HeaderText = "會議室名稱"
        Room_memo.HeaderText = "備註"
        ...
        '設定排序所對應的欄位
        room_code.SortExpression = "room_code"
        usr_code.SortExpression = "usr_code"
        Room_name.SortExpression = "Room_name"
        Room_memo.SortExpression = "Room_memo"
        Is_use.SortExpression = "Is_uses"

        '欄位寬度(%):要留6%給編輯圖示
        room_code.ItemStyle.Width = Unit.Percentage(20)
        usr_code.ItemStyle.Width = Unit.Percentage(10)
        Room_name.ItemStyle.Width = Unit.Percentage(30)
        Room_memo.ItemStyle.Width = Unit.Percentage(30)
        Is_use.ItemStyle.Width = Unit.Percentage(10)

        '設定欄位標題顏色
        room_code.HeaderStyle.CssClass = "grid1"
        usr_code.HeaderStyle.CssClass = "grid1"
        Room_name.HeaderStyle.CssClass = "grid1"
        Room_memo.HeaderStyle.CssClass = "grid1"
        Is_use.HeaderStyle.CssClass = "grid1"

        '設定欄位的水平對齊
        room_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        usr_code.ItemStyle.HorizontalAlign = HorizontalAlign.Center
        Room_name.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Room_memo.ItemStyle.HorizontalAlign = HorizontalAlign.Left
        Is_use.ItemStyle.HorizontalAlign = HorizontalAlign.Center

        '設定欄位是否顯示(不顯示的再設定)
        'room_code.Visible = False

        GridView1.Columns.Add(room_code)
        GridView1.Columns.Add(usr_code)
        GridView1.Columns.Add(Room_name)
        GridView1.Columns.Add(Room_memo)
