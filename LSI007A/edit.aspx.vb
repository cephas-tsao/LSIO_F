...
Partial Class basic_LSI007A_edit
    Inherits PageBase
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        '自動編號
        'If txtPgmType.Text <> "MDY" Then txt_room_code.Text = "AS" & Format(Now, "yyMMdd") & Right("0000" & Get_AutoIntMax("Mail_Asklaw", "RIGHT(ask_code, 10)", " WHERE ask_code LIKE 'AS" & Format(Now, "yyMMdd") & "%'") + 1, 4)
        ...
        If SaveEditData(s_prgcode, Pgm_Type, txt_room_code.Text, txt_room_name.Text, ddl_is_use.SelectedValue, _
                        txt_room_memo.Text, Session("usr_code"), Format(Now, "yyyy/MM/dd")) Then
            '儲存使用紀錄
            ...
            If Pgm_Type <> "MDY" Then
                ...
                If txtSubWhere.Text <> "" Then
                    txtSubWhere.Text &= " OR room_code='" & RepSql(sKeyPrimary) & "' "
                Else
                    txtSubWhere.Text = " WHERE room_code='" & RepSql(sKeyPrimary) & "' "
                End If
            End If
            '存檔正確導回default頁面
    ...
    Protected Function Chk_data() As Boolean
        ...
        If txtPgmType.Text = "ADD" Then
            If Chk_RelData(s_prgcode, "", txt_room_code.Text) = False Then
                Chk_data = False
        ...
        ''檢查必填欄位是否空白
        If txt_room_name.Text = "" Or txt_room_code.Text = "" Then
            Chk_data = False
            ...
            Select Case Pgm_type
                Case "MDY", "COPY"
                    ...
                    If Dt_tmp.Rows.Count > 0 Then
                        txt_room_code.Text = Trim(Dt_tmp.Rows(0).Item("room_code") & "")
                        txt_room_name.Text = Trim(Dt_tmp.Rows(0).Item("room_name") & "")
                        ddl_is_use.SelectedValue = Trim(Dt_tmp.Rows(0).Item("is_use") & "")
                        txt_room_memo.Text = Trim(Dt_tmp.Rows(0).Item("room_memo") & "")
                    End If

                    If Pgm_type = "COPY" Then
                        txt_room_code.Text = ""
                        txt_room_code.Enabled = False
                    Else
                        txt_room_code.Enabled = False
                    End If
            End Select
